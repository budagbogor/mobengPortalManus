...
function App() {
  ...
  const handleSaveApiKey = async (apiKey: string) => {
    try {
      // API Key sudah disimpan di localStorage oleh ApiSettingsModal
      setApiKeyMessage('API Key berhasil diatur!');
      setTimeout(() => setApiKeyMessage(null), 3000);
    } catch (error) {
      console.error('Error saving API key:', error);
      setApiKeyMessage('Gagal menyimpan API Key');
    }
  };
  ...
  if (currentView === 'role_selection') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-mobeng-darkblue to-mobeng-blue flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans">
        ...
        {/* API Key Alert */}
        {!apiKeyMessage && (
          <div className="absolute top-4 right-4 z-20 bg-amber-500/90 backdrop-blur-sm text-white px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 cursor-pointer hover:bg-amber-600 transition-colors" onClick={openApiSettings}>
            <AlertTriangle size={16} />
            <span>⚙️ Setup API Key Gemini</span>
          </div>
        )}
        ...
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl w-full">
            {/* Settings Button */}
            <button onClick={openApiSettings} className="absolute top-4 right-4 md:top-8 md:right-8 p-3 bg-white/10 hover:bg-white/20 border border-white/20 rounded-full transition-all duration-300 text-white" title="Pengaturan API Key">
                <Settings size={20} />
            </button>
            <button onClick={() => setCurrentView('recruiter_login')} className="group relative bg-white/10 backdrop-blur-md border border-white/20 hover:bg-white/20 hover:border-mobeng-blue/50 rounded-2xl p-8 transition-all duration-300 flex flex-col items-center text-center shadow-xl">
                <div className="w-20 h-20 bg-gradient-to-br from-mobeng-blue to-mobeng-darkblue rounded-full flex items-center justify-center mb-6 shadow-lg shadow-mobeng-blue/20 group-hover:scale-110 transition-transform">
                    <Lock className="text-white w-10 h-10" />
                </div>
                <h2 className="text-2xl font-bold text-white mb-2">Akses Recruiter</h2>
                <p className="text-blue-50 text-sm mb-6 font-medium">Lihat hasil seleksi, analisis kandidat, dan kirim hasil via WhatsApp.</p>
                <div className="text-white/70 text-xs mb-4 px-4 py-2 bg-black/20 rounded-lg">
                    <span className="font-bold">Fitur:</span> Dashboard <ArrowRight size={10} className="inline"/> Analytics <ArrowRight size={10} className="inline"/> Export
                </div>
                <span className="text-mobeng-blue bg-white py-2 px-4 rounded-full font-bold text-sm flex items-center gap-2 group-hover:translate-x-1 transition-transform shadow-lg">
        ...
        <div className="relative z-10 mt-8 text-center flex items-center justify-center gap-6">
            <button onClick={() => openDocs('candidate')} className="text-blue-50 hover:text-white text-sm font-medium flex items-center gap-2 transition-colors">
                <HelpCircle size={16} />
                Bantuan & Panduan
            </button>
            <span className="text-white/30">|</span>
            <button onClick={openApiSettings} className="text-blue-50 hover:text-white text-sm font-medium flex items-center gap-2 transition-colors">
                <Server size={16} />
                Pengaturan API
            </button>
        </div>
